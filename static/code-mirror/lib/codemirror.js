...
;(function(global, factory) {
  ...
  function alignHorizontally(cm) {
    ...
    var comp =
      compensateForHScroll(display) - display.scroller.scrollLeft + cm.doc.scrollLeft
  ...
  function attachLocalSpans(doc, change, from, to) {
    var existing = change['spans_' + doc.id],
      n = 0
    doc.iter(Math.max(doc.first, from), Math.min(doc.first + doc.size, to), function(
      line
    ) {
      if (line.markedSpans) {
  ...
  function findSharedMarkers(doc) {
    return doc.findMarks(Pos(doc.first, 0), doc.clipPos(Pos(doc.lastLine())), function(
      m
    ) {
      return m.parent
    })
      ...
      if (behavior.unit == 'rectangle') {
        ...
        for (
          ...
          if (left == right) {
            ...
            ranges.push(
              new Range(Pos(line, leftPos), Pos(line, findColumn(text, right, tabSize)))
            )
          }
        }
        if (!ranges.length) {
